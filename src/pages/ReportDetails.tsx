...
const Report: React.FC = () => {
  ...
  const formatElevation = (test: any) => {
    if (!test.elevationValue && !test.elevationReference) return 'Not specified';
    const value = test.elevationValue ? `${test.elevationValue}${test.elevationUnit || ''}` : '';
    const reference = test.elevationReference ? ` ${test.elevationReference}` : '';
                ...
                {reportData.photos.map((photo) => (
                  ...
                  >
                    <Box
                      ...
                      alt="Report photos"
                      src={photo.url || "https://placehold.co/100x100?text=No+Image"}
                    />
                    <Box>
                      <Typography variant="h6">Figure {photo.id}</Typography>
                      <Typography variant="body2">{photo.description || 'No description'}</Typography>
                      <Typography variant="body2">{photo.code || 'No code'}</Typography>
